...
describe("control-plane snapshot", () => {
  ...
  it("bounds an adapter that never settles", async () => { vi.useFakeTimers(); const pending = buildControlPlaneSnapshot("better-palia-maps", deps({ never: true })); await vi.advanceTimersByTimeAsync(SNAPSHOT_DEADLINE_MS); const result = await pending; expect(result.sources.herdr.error?.code).toBe("timeout"); vi.useRealTimers(); });
