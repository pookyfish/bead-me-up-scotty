...
- The spike owns a pure per-channel loop-guard state machine and an operator-invoked autonomous-send gate. Autonomous requests 1–5 transition `active(0)` through `active(5)` before MCP; the sixth is allowed but transitions `active(5)` to `paused(6)` before MCP; a seventh from `paused(6)` is locally rejected, remains paused, and must have no upstream request/message evidence. Only directly evidenced authenticated human-origin activity transitions `paused(6)` to `active(0)`; `/continue`, agent-origin text, and unauthenticated events never reset it.
...
| `docs/superpowers/evidence/2026-08-09-scotty-agentchattr-compatibility-spike/provenance.md` | Already-created, **approval-conditional** pre-execution pin, Windows compatibility, license/hash, upstream server/API/MCP version or explicit absence, and copy/attribution conclusion. Its current status is `HOLD`/pending independent review; once approved it is read-only to the spike branch. |
...
- [ ] **Step 2: Write failing identity and loop-guard tests**
  ...
  Define the pure per-channel state machine and use this exact transition diagram:

  ```text
  active(0) --autonomous allow--> active(1) --autonomous allow--> ... --> active(5)
  active(5) --sixth autonomous allow, before MCP--> paused(6)
  paused(6) --seventh autonomous request--> paused(6) + rejected-before-MCP
  paused(6) --authenticated human-origin proof--> active(0)
  paused(6) --/continue | agent-origin | unauthenticated event--> paused(6)
  ```

  `requestAutonomousSend(channel, evidence)` returns an allow record only from `active(0)` through `active(5)`, transitions the sixth allowed request from `active(5)` to `paused(6)` **before** its MCP call, and returns a rejected-before-MCP record from `paused(6)` without an MCP invocation. `recordAuthenticatedHumanOrigin(channel, evidence)` transitions only `paused(6)` to `active(0)` when it carries direct upstream UID, channel, timestamp, authenticated human-origin proof, and a verified human identity. Agent text, an unauthenticated event, `/continue`, and an upstream control never reset. Test six consecutive allowed agent-originated disposable send requests, the sixth pre-MCP transition to `paused(6)`, a seventh rejection that remains paused and has no MCP invocation/upstream UID, the human-only reset, and every invalid reset source.
...
- [ ] **Step 4: Run tests and implement only spike-contract validation**
  ...
  Run `npm run test:unit -- tools/agentchattr-compatibility-spike/spike-contract.test.ts`, confirm failure, then implement validation without adding a Scotty provider or calling a production route. Implement the operator-invoked `Invoke-GuardedAutonomousSend` wrapper in `run-spike.ps1` around every autonomous test MCP send; it must persist its pre-send decision, allow the sixth only after recording `active(5) -> paused(6)` before MCP, and refuse the seventh without issuing an MCP request while retaining `paused(6)`. If any autonomous send can bypass interception, do not run autonomous live cases: record `unsupported` and `NO-GO`. Re-run focused and full tests; expected result is pass for the harness logic, not a claim that upstream supplies every semantic.
